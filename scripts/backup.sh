...
EOF
    ...
    # 压缩备份
    log_info "压缩备份..."
    local archive_file="${BACKUP_ROOT_DIR}/${timestamp}_${description}.tar.gz"
...
cleanup_old_backups() {
    local backups=($(ls -t "${BACKUP_ROOT_DIR}"/*.tar.gz 2>/dev/null))
...
restore_backup() {
    ...
    # 解压备份
    local temp_dir="/tmp/singbox_restore_$$"
    mkdir -p "$temp_dir"
    ...
    if [[ -d "${extracted_dir}/data" ]]; then
        ...
        cp -r "${extracted_dir}/data" "${SCRIPT_DIR}/"
        check_item "数据文件" "ok"
    fi
