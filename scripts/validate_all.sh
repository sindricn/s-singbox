...
# main 在线安装入口必须克隆 main，并保留显式环境变量覆盖，防止 process substitution 选错分支。
grep -q '^DEFAULT_BRANCH="main"$' "$ROOT_DIR/install.sh"
...
grep -q 'SINGBOX_KERNEL_CHANNEL_TAG="${SINGBOX_KERNEL_CHANNEL_TAG:-kernel-stable}"' "$ROOT_DIR/modules/zz_singbox_114.sh"
...
grep -q 'SagerNet/sing-box/releases/latest' "$ROOT_DIR/.github/workflows/validate-custom-kernel.yml"
grep -q 'kernel-manifest.json' "$ROOT_DIR/.github/workflows/validate-custom-kernel.yml"
grep -q 'STABLE_TAG: kernel-stable' "$ROOT_DIR/.github/workflows/validate-custom-kernel.yml"
grep -q "should_build:.*steps.metadata.outputs.should_build" "$ROOT_DIR/.github/workflows/validate-custom-kernel.yml"
grep -q "SINGBOX_BUILD_TEST_VERSION:?SINGBOX_BUILD_TEST_VERSION is required" "$ROOT_DIR/scripts/validate_custom_kernel_build.sh"
